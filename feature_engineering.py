...
if __name__ == "__main__":
    ...
    input_file = _resolve_json_path('merged_cleaned_data.json')
    output_file = os.path.join(script_dir, 'json', 'engineered_features.json')

    if not os.path.exists(input_file):
        raise FileNotFoundError(
            "Không tìm thấy merged_cleaned_data.json. "
            f"Đã thử: {os.path.join(script_dir, 'json', 'merged_cleaned_data.json')} và "
            f"{os.path.join(os.path.dirname(script_dir), 'json', 'merged_cleaned_data.json')}."
        )
